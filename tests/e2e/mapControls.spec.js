...
test('control-block drives map area changes', async ({ page }) => {
  ...
  const controlPanel = page.locator('#map-controls .tanvis-controls');
  await expect(controlPanel).toHaveCount(1);
  await expect(controlPanel.locator('.tanvis-controls-toggle')).toHaveAttribute('aria-expanded', 'true');
  await expect(controlPanel.locator('.tanvis-controls-toggle')).toContainText('Data options');
  await expect(page.getByLabel('vc60')).toBeChecked();
  await expect(controlPanel).not.toContainText('Area');

  await controlPanel.locator('.tanvis-controls-toggle').click();
  await expect(controlPanel.locator('.tanvis-controls-toggle')).toHaveAttribute('aria-expanded', 'false');
  await expect(controlPanel.locator('.tanvis-controls-group')).toBeHidden();

  await controlPanel.locator('.tanvis-controls-toggle').click();
  await expect(controlPanel.locator('.tanvis-controls-toggle')).toHaveAttribute('aria-expanded', 'true');
  await expect(controlPanel.locator('.tanvis-controls-group')).toBeVisible();
