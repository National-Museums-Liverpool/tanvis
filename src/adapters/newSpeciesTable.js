...
export function createNewSpeciesTableAdapter() {
  return {
    ...
    render(element, config) {
      ...
      clearElement(element);
      const summary = createSummary(startDate, endDate, 0, renderConfig.region);
...
function createSummary(startDate, endDate, count, region, taxonGroupName) {
  const summary = document.createElement('div');
  summary.classList.add('tanvis-table-header-text');
  summary.textContent = buildSummaryText(startDate, endDate, count, region, taxonGroupName);
  return summary;
}

function buildSummaryText(startDate, endDate, count, region, taxonGroupName) {
  const suffix = taxonGroupName ? ` for taxon group ${taxonGroupName}` : '';
  return `${count} new species between ${startDate} and ${endDate} for ${formatTableRegionLabel(region)}${suffix}`;
}
...
function refreshSummary(element, labelMode) {
  ...
  summary.textContent = buildSummaryText(state.startDate, state.endDate, state.count, state.region, taxonGroupName);
}
